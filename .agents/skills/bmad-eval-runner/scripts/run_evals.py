...
For each eval, the runner:
  ...
  3. Copies the skill into .llm/skills/ so it is discoverable by llm.
  4. Stages any fixture files declared in the eval's `files` list.
  5. Runs `llm -p '<prompt>' --output-format stream-json --verbose`, capturing
  ...
  6. Rsyncs any files llm wrote into `<run-dir>/<eval-id>/artifacts/`.
...
def stage_workspace_local(
    ...
    skill_link_dir = project_dest / ".llm" / "skills"
...
cp -R "$SKILL_SRC" "/workspace/.llm/skills/$SKILL_NAME"
if [ -d /fixtures ]; then
...
rsync -a --exclude=.llm --exclude=node_modules --exclude=.git \
  --filter='+ */' --filter='+ *' \
...
def sync_artifacts(workspace: Path, new_files: list[str], dest: Path) -> None:
    for rel in new_files:
        ...
        if not src.is_file():
            continue
        if any(part in (".llm", "node_modules", ".git", ".venv") for part in src.parts):
            continue
        target = dest / rel
