...
def read_macos_keychain_credentials() -> str | None:
    """Read the automation assistant OAuth credentials JSON from the macOS Keychain.

    Returns the raw JSON string stored under service "automation assistant-credentials",
    ...
    can be staged into each isolated workspace's `.llm/.credentials.json` before
    `llm -p` is launched. Without this, an isolated subprocess with HOME pointed
    ...
    try:
        result = subprocess.run(
            ["security", "find-generic-password", "-s", "automation assistant-credentials", "-w"],
            capture_output=True,
            timeout=5,
        )
...
def stage_credentials(llm_dir: Path, credentials_json: str | None) -> None:
    """Write credentials_json to <llm_dir>/.credentials.json. No-op if None."""
    if not credentials_json:
        return
    llm_dir.mkdir(parents=True, exist_ok=True)
    (llm_dir / ".credentials.json").write_text(credentials_json, encoding="utf-8")
