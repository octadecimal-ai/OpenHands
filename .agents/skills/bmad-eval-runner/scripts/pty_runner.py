...
# requires-python = ">=3.9"
# ///
"""Run llm interactively via PTY so the Skill tool is available.

In `llm -p` (print mode) the Skill tool is never offered — LLM handles
everything inline. Running `llm` in interactive mode activates the Skill
tool so dependency skills installed in .llm/skills/ can be properly invoked.

The PTY tricks llm into thinking it has a terminal (interactive mode) while
...
# How long to wait for llm to initialize before sending the prompt.
# LLM loads skill registry, checks credentials, etc. on startup.
INIT_WAIT_S = 5.0

# How long to wait after the stream-json 'result' event before killing llm.
...
def run_interactive(prompt: str, output: Path, timeout: int = 600) -> None:
    """Spawn llm interactively via PTY, send one prompt, capture transcript."""
    ...
    try:
        while True:
            ...
            if r:
                ...
                while b"\n" in buf:
                    ...
                    try:
                        obj = json.loads(line)
                        # 'result' marks end of a llm turn.
                ...
                # Silence window — send prompt once llm has had time to init.
        ...
        # Politely ask llm to exit, then hard-kill if needed.
        try:
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Run llm interactively via PTY and capture stream-json transcript"
    )
