...
- **Trigger evals** (`triggers.json`) — measure whether the skill's `description` actually causes LLM to invoke the skill on a given query versus stay clear when it shouldn't.
...
- **Docker** (preferred when available): each eval runs in a fresh container off `bmad-eval-runner:latest`. The host's `LLM_PROVIDER_API_KEY` is the only env passed in. The skill's project is bind-mounted read-only and copied into a writable scratch dir inside the container; `HOME` is a fresh in-container directory; there is no auto-memory and no host AGENTS.md.
...
For artifact evals, invoke `python3 {skill-root}/scripts/run_evals.py` with the resolved arguments. The script handles isolation per eval, runs `llm -p` in the sandbox with the eval's prompt and any staged fixture files, and writes a per-eval folder with `prompt.txt`, `transcript.jsonl`, `artifacts/`, and `metrics.json`.
...
- **Auth boundary is narrow.** On macOS, the host's automation assistant OAuth credential is staged into each isolated `.llm/.credentials.json` so the subprocess can authenticate without inheriting host config. `LLM_PROVIDER_API_KEY`, if set, is also forwarded. Nothing else crosses.
