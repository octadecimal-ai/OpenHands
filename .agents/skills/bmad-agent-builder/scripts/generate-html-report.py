...
.section-header { display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem 1rem; background: var(--surface); ide: pointer; user-select: none; }
...
.btn { background: none; border: 1px solid var(--border); border-radius: 0.25rem; padding: 0.3rem 0.7rem; ide: pointer; color: var(--text-muted); font-size: 0.8rem; transition: all 0.15s; }
...
function renderCapabilities() {
  ...
  caps.forEach((cap, idx) => {
    ...
    html += `<div class="cap-row" ${hasFindings ? `onclick="toggleCapFindings(${idx})" style="ide:pointer"` : ''}>`;
