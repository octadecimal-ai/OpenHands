...
Skills may run in environments where Python or `uv` is unavailable (e.g., llm.ai web). Scripts should be the fast, reliable path — but the skill must still deliver its outcome when execution is not possible.
