...
def test_basic_scaffold():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (tmp / ".llm-plugin" / "marketplace.json").is_file()
...
def test_marketplace_json_content():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        marketplace = json.loads(
            (tmp / ".llm-plugin" / "marketplace.json").read_text()
        )
...
def test_custom_marketplace_dir():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (custom_dir / ".llm-plugin" / "marketplace.json").is_file()
        assert not (tmp / ".llm-plugin" / "marketplace.json").exists()
        assert data["marketplace_json"] == str((custom_dir / ".llm-plugin" / "marketplace.json").resolve())
