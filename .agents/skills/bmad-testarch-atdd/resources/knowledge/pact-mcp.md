...
| ----------------- | ------------------------------------- | ---------------------- |
| automation assistant       | `~/.llm.json`                      | JSON (`mcpServers`)    |
| Codex             | `~/.config/agent-cli/config.toml`                | TOML (`[mcp_servers]`) |
| Gemini CLI        | `~/.gemini/settings.json`             | JSON (`mcpServers`)    |
| IDE            | `~/.ide/mcp.json`                  | JSON (`mcpServers`)    |
| Windsurf          | `~/.codeium/windsurf/mcp_config.json` | JSON (`mcpServers`)    |
| VS Code (Copilot) | `.vscode/mcp.json`                    | JSON (`servers`)       |

> **automation assistant tip**: Prefer the `llm mcp add` CLI over manual JSON editing. Use `-s user` for global (all projects) or omit for per-project (default).

### CLI shortcuts (automation assistant and Codex)

```bash
# automation assistant — use add-json for servers with env vars (-s user = global)
llm mcp add-json -s user smartbear \
...
### JSON config (Gemini CLI, IDE, Windsurf)
...
Agent CLI uses TOML instead of JSON. Add to `~/.config/agent-cli/config.toml`:
