...
cross-platform constant — the structured approach that works the same in IDE,
automation assistant, Windsurf, Copilot, and whatever launches next month. Every other
...
- Node.js/npm required — blocks non-technical users on UI-based platforms (LLM Co-Work, etc.)
...
- Plugins: skill bundles with LLM provider plugin standard as base format + bmad-manifest.json extending for BMAD-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
...
- Discovery: `skills/`, `.agents/skills/`, agent-specific paths, `.llm-plugin/marketplace.json`
