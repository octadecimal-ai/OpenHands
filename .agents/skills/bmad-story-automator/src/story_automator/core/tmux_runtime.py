...
def agent_type() -> str:
    return os.environ.get("AI_AGENT", "llm")


def agent_cli(agent: str) -> str:
    return "codex exec" if agent == "codex" else "llm --dangerously-skip-permissions"
...
def _runner_session_status(
    ...
    *,
    ...
    if _runner_llm_prompt_completed(paths, state, capture, prompt_visible):
        state = load_session_state(paths.state)
        return _terminal_runner_status(session, state, full=full, project_root=root)

    if lifecycle == "running" and child_alive:
        label = active_task or ("Codex working" if codex else "LLM working")
        return {
            "status": "active",
    ...
    if lifecycle in {"created", "launching"} or runner_alive or pane.exists:
        label = active_task or ("launching codex" if codex else "launching llm")
        return {
            "status": "idle",
            "todos_done": todos_done,
            "todos_total": todos_total,
...
def _runner_llm_prompt_completed(
    paths: SessionPaths,
    state: dict[str, object],
    capture: str,
    prompt_visible: str,
) -> bool:
    if str(state.get("agent") or "") != "llm":
        return False
    if str(state.get("lifecycle") or "") != "running":
        return False
    if prompt_visible != "true":
        return False
    if not _llm_completion_marker_present(capture):
        return False
...
def _llm_completion_marker_present(capture: str) -> bool:
    if not capture:
        return False
    return bool(
        re.search(
...
def _legacy_session_status(
    ...
    *,
    ...
    return _legacy_llm_session_status(session, full=full, project_root=project_root)
...
def _legacy_llm_session_status(
    session: str,
    *,
    full: bool,
    project_root: str | None,
    ...
    pane_pid = _safe_int(tmux_display(session, "#{pane_pid}"))
    llm_running = pane_pid > 0 and run_cmd("pgrep", "-P", str(pane_pid), "-f", "llm")[1] == 0
    ...
    if activity_detected or llm_running:
        active_task = extract_active_task(capture) or "LLM working"
...
def _legacy_heartbeat_check(session: str, selected_agent: str) -> tuple[str, float, str, str]:
    ...
    pattern = "codex" if selected_agent == "codex" else "llm"
