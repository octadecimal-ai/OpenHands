...
|------|-------------|---------------|
| `create` | Create story from epic | LLM, Codex |
| `dev` | Implement story tasks | LLM, Codex |
| `auto` | Test automation | LLM, Codex |
| `review` | Code review with auto-fix | LLM, Codex |
| `retro` | Retrospective (YOLO mode) | **LLM ONLY** |

## Retrospective Commands (v1.5.0)

**CRITICAL:** Retrospectives use a special step type that:
- Always uses LLM (Codex not supported)
...
cmd=$("$scripts" tmux-wrapper build-cmd retro {epic_number} --agent "llm")
session=$("$scripts" tmux-wrapper spawn retro "" {epic_number} --agent "llm" --command "$cmd")

# Monitor (retrospectives never block, failures just logged)
result=$("$scripts" monitor-session "$session" --json --agent "llm")
"$scripts" tmux-wrapper kill "$session"
```
