...
**Purpose:** Prevent ALL false "stuck" escalations by using the automation assistant statusline as definitive proof-of-life.

### How It Works

automation assistant displays a statusline at the bottom of the terminal:
```
folder | ctx(N%) | HH:MM:SS
                   ^^^^^^^^ <- This time updates continuously while LLM runs
```
