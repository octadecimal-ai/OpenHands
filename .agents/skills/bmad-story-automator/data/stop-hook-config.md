...
This document defines the automation assistant Stop hook required for the story-automator to prevent premature stopping during orchestration.
...
2. When LLM tries to stop → Hook script checks marker file
3. If no marker or completed → Allow stop (normal LLM usage)
...
**New location:** `{project_root}/.llm/.story-automator-active`
...
- New: Each project has its own marker in `.llm/` → Full isolation

### How It Works

1. Stop hook uses LLM's current working directory as the active project root
2. Run the orchestrator from the project root so the marker resolves to `{project_root}/.llm/.story-automator-active`
...
Add this to the target project's `.llm/settings.json`:
...
- Check the binary exists: `test -x .llm/skills/bmad-story-automator/scripts/story-automator`
- Ensure binary is executable: `chmod +x .llm/skills/bmad-story-automator/scripts/story-automator`

---

## Marker File Format

**Location (v2.0):** `{project_root}/.llm/.story-automator-active`

*Note: Ensure `.llm/.story-automator-active` is in your `.gitignore`*
...
```
1. Check if .llm/settings.json exists
