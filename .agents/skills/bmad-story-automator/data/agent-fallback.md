...
**Multi-Agent Support:** The orchestrator can use LLM or Codex as AI coding agents, with automatic fallback on failure.
...
|-------|-----|--------------|---------|---------------|
| LLM | `llm --dangerously-skip-permissions` | Natural language skill prompt | 60min | ☒/☐ checkboxes |
| Codex | `codex exec --full-auto` | Natural language prompt | 90min (1.5x) | Not supported |

**CRITICAL:** Both LLM and Codex prompts must name the skill/workflow to execute and include the story ID.
...
- **Longer waits:** Status check script returns 90s wait estimate for Codex (vs 60s for LLM)
...
agentConfig:
  ...
  perTask:
    review:
      primary: "llm"   # Never use Codex for code-review
      fallback: false
```
...
- If `agentConfig` is missing, default to LLM-only (no fallback)
