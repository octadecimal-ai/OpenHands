...
**CRITICAL:** The stop hook is installed at the PROJECT level. When the orchestrator spawns T-Mux sessions (create-story, dev-story, code-review), those child LLM instances:
1. Run in the same project directory
2. Read the same `.llm/settings.json`
...
## Internal LLM Calls (Session 20 Fix)

**CRITICAL:** Scripts that internally call `llm` (like `story-automator tmux-status-check` using Haiku for wait estimation) MUST prefix the call with the environment variable:

```bash
# WRONG - will hang when stop hook blocks the llm exit
RESULT=$(llm -p --model haiku "..." 2>/dev/null)

# CORRECT - allows llm to exit normally
RESULT=$(STORY_AUTOMATOR_CHILD=true llm -p --model haiku "..." 2>/dev/null)
```

**Why:** Even non-interactive `llm -p` calls trigger the stop hook when they exit. Without the env var, the hook sees the marker file and blocks, causing the script to hang indefinitely.
...
**Why this happens:** The stop hook fires when LLM pauses, not just when explicitly stopping. During menu waits, it may fire repeatedly. Ignore these messages when waiting for user input.
...
1. Remove the marker file: `rm .llm/.story-automator-active` (from project root)
2. Stop LLM normally
...
rm -f .llm/.story-automator-active
...
| "no such file" | BMAD installed? Path correct? `ls -la .llm/skills/bmad-story-automator/scripts/` |
...
| Script hangs | Internal llm calls missing env var? See Session 20 Fix. |
