...
|------------|--------|-----|------|--------|-----------|
| Low | llm | llm | llm | llm | LLM handles simple tasks well |
| Medium | codex | codex | codex | codex | Codex for moderate complexity (LLM fallback) |
| High | codex | codex | codex | codex | Codex for complex work (LLM fallback) |
| Retro | llm | - | - | - | Retrospectives always use LLM |
...
|------------|--------|-----|--------|-----------|
| Low | llm | llm | llm | LLM handles simple tasks well |
| Medium | codex | codex | codex | Codex for moderate complexity (LLM fallback) |
| High | codex | codex | codex | Codex for complex work (LLM fallback) |
| Retro | llm | - | - | Retrospectives always use LLM |
...
Enter a name to save (e.g., `all-llm`, `codex-heavy`) or [N]o to skip:
```
...
- `llm` - LLM for everything (more capable, slower)
- `codex` - Codex for everything (faster, simpler)
- `llm, false` - LLM only, no fallback
...
- `low.dev: llm, false` → LLM for low-complexity dev, no fallback
- `medium.create: codex, llm` → Codex for medium-complexity create
- `high.review: llm, false` → LLM for high-complexity review

**Per-Task Format (applies to all complexities):** `task: primary, fallback`
- `review: llm, false` → LLM for ALL reviews
- `dev: codex, llm` → Codex for ALL dev
