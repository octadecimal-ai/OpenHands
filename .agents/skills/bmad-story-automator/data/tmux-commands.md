...
Script: resolve the installed helper under `.llm/skills/bmad-story-automator/scripts/story-automator`

```bash
# ALWAYS use absolute path - relative paths break when directory changes
script="$(printf "%s" "{project_root}/.llm/skills/bmad-story-automator/scripts/story-automator")"
...
2. `todos_done` - completed todo count (LLM only; Codex returns 0)
3. `todos_total` - total todo count (LLM only; Codex returns 0)
...
The status check script automatically detects LLM vs Codex sessions:
- **LLM:** Looks for `ctrl+c to interrupt`, `☒`/`☐` checkboxes
...
tmux new-session -d -s "SESSION_NAME" -x 200 -y 50 -c "PROJECT_PATH" \
  -e STORY_AUTOMATOR_CHILD=true -e AI_AGENT=codex -e AGENTSCODE= -e BASH_ENV= \
  /bin/sleep 86400
...
| Variable | LLM | Codex |
|----------|--------|-------|
| CLI | `llm --dangerously-skip-permissions` | `codex exec --full-auto` |
...
- `AI_AGENT` = `llm` or `codex` (used by story-automator tmux-wrapper and story-automator monitor-session)
