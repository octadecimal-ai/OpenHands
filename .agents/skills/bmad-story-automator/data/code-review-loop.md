...
agentConfig:
  ...
  perTask:
    review:
      primary: "llm"      # Override: use LLM for reviews
...
```bash
scripts="$(printf "%s" "{project_root}/.llm/skills/bmad-story-automator/scripts/story-automator")"
...
- If `reviewCycle > maxCycles`: Escalate with CRITICAL priority (Trigger #8), then present options:
  ...
  2. **[2] Run with LLM** - Re-run code-review with LLM agent
