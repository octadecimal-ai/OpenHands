...
cmd=$("{scriptsDir}" tmux-wrapper build-cmd retro {epic_number} --agent "llm")
session=$("{scriptsDir}" tmux-wrapper spawn retro "" {epic_number} --agent "llm" --command "$cmd")

# Monitor with safe failure (never escalate on retro failure)
retro_timeout=60
[ "$story_count" -gt 10 ] && retro_timeout=90
result=$("{scriptsDir}" monitor-session "$session" --json --agent "llm" --timeout "$retro_timeout")
...
- **Use `build-cmd retro` with LLM**: Retrospectives do not support Codex
