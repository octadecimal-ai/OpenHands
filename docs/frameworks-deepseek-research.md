# Raport rozpoznawczy: frameworki orkiestracji multi-agent dla Claude Code
...
- **Ruflo (dawniej Claude Flow) jest najbliżej gotowego produktu** – 31k+ gwiazdek, 60+ agentów, 215 narzędzi MCP, self-learning memory – ale governance to wciąż ręczne promptowanie, a nie formalny rulebook.
- **OpenHands ma najlepszy model sandboxingu kontenerowego** – Docker z wieloma runtime'ami, MIT license, SDK dla agentów – ale Claude Code jest do niego podpinany przez ACP (Agent-Client Protocol), integracja dopiero się stabilizuje.
- **Mastra to cichy lider dla warstwy TypeScript** – Apache 2.0, observability, workflows, wsparcie Claude – świetnie pasuje do stacku Octadecimal.
- **Natywne Agent Teams Claude Code zmienia reguły gry.** Wszystkie nakładki orkiestracyjne, które duplikują funkcję już wbudowaną w Claude Code, tracą rację bytu – frameworki muszą robić coś ponad to (governance, observability, pamięć międzysesyjną).
...
|---|---|---|---|---|---|
| **Ruflo (ex Claude Flow)** | [github.com/ruvnet/ruflo](https://github.com/ruvnet/ruflo) | Aktywny (5.8k+ commitów) | 2026-03-27 (v3.5.0) | MIT | 20 |
| **Claude-Squad (bijutharakan)** | [github.com/bijutharakan/multi-agent-squad](https://github.com/bijutharakan/multi-agent-squad) | Średnio aktywny | 2025-12-14 | Brak jawnie wskazanej | 12 |
...
| **SuperClaude** | [PyPI: superclaude](https://pypi.org/project/superclaude/) | Aktywny (v4.3.0) | 2026-03-22 | MIT | 15 |
...
| **Cline (ex Claude Dev)** | [github.com/cline/cline](https://github.com/cline/cline) | Bardzo aktywny | 2026-05 (ciągłe) | Apache 2.0 | 13 |
...
### 3.1 Ruflo (dawniej Claude Flow) – najbliżej kompletnego produktu

Ruflo to najbardziej dojrzały framework orkiestracji agentów dla Claude Code. Powstał jako osobisty eksperyment Reuvena Cohena w maju 2025, a do lutego 2026 przeszedł pełny rebranding z Claude Flow na Ruflo z całkowitą przebudową architektury na Rust/WASM. Obecnie ma 31k+ gwiazdek na GitHubie, 100k+ aktywnych użytkowników miesięcznie i 5800+ commitów.
...
**Architektura:** Event-driven z Agent-Client Protocol (ACP). OpenHands pełni rolę warstwy abstrakcji – możesz podpiąć Claude Code, Gemini CLI, Codex CLI jako back-endy agentowe, zachowując wspólny interfejs. Trwają intensywne prace nad zrównaniem API delegacji z natywnym Claude Code Task tool.
...
**Ostre krawędzie:** Heavy dependency – 70+ pip packages. Legacy system (V0) wciąż obecny, z planowanym usunięciem na kwiecień 2026, co sugeruje pewną niestabilność architektoniczną w okresie przejściowym. Integracja z Claude Code przez ACP jest w fazie implementacji, nie produkcji. Młode SDK (4 miesiące od premiery) – brak referencji produkcyjnych.
...
**Architektura:** Trzy filary: Agents (LLM + tools + workflows), Workflows (durable graph-based state machines z OpenTelemetry tracing), RAG (ETL pipeline z vector search). Model-agnostic przez Vercel AI SDK – Claude, GPT, Gemini, Llama przez jeden interfejs. Agents mogą wywoływać inne agents, tworząc hierarchię. Każdy krok workflow ma wbudowany tracing OpenTelemetry.

**Claude Code integration:** Nie jest to first-class integration w sensie "uruchamiam Claude Code CLI". Mastra używa Claude przez API Anthropic, a nie przez Claude Code. Dla Octadecimal to oznacza dodatkową warstwę integracji – chyba że akceptujesz model API-zamiast-CLI.
...
**Architektura:** Role-based agents (Agent, Task, Crew, Process). Dwa tryby: Crews (collaborative multi-agent groups) i Flows (event-driven, stateful). Pamięć short/long-term. Integracja z Claude przez API Anthropic, nie przez Claude Code CLI. Dostępny jest "Agent Workflow Designer" jako skill dla Claude Code, ale to zewnętrzny dodatek.
...
**Ostre krawędzie:** Python-only – nie pasuje do stacku TypeScript dla MCP. Brak integracji z Claude Code jako CLI – tylko API. "Agents still have no schema" – krytyka, że brakuje formalnej walidacji outputów agentów. Do pełnej observability potrzebuje zewnętrznych narzędzi (LangSmith, Arize, Langfuse).

### 3.5 Cline + Roo Code – IDE-first, nie platforma

Oba to rozszerzenia VS Code (Apache 2.0). Cline (58k+ gwiazdek) jest oryginałem, Roo Code (22k+ gwiazdek) forkiem. Cline potrafi spawn-ować Claude Code CLI jako sub-agenty, co jest ciekawym wzorcem orkiestracji, ale fundamentalnie są to narzędzia IDE, a nie platformy orkiestracyjne.

**Cline:** Wieloagentowa orkiestracja przez `claude-code-subagents.md` rule. Spawnuje Claude Code CLI z różnymi permission modes (w tym `bypassPermissions`). Wzorce: Parallel Variations (Midjourney for Code), Parallel Features on Existing Codebase. Ciekawe dla fazy prototypowania, za lekkie dla platformy produkcyjnej.
...
| **Mastra** | **Buy** (adoptować jako warstwę orkiestracji TypeScript) | Apache 2.0, TypeScript, observability, workflows. Dobrze pasuje do stacku. Brakuje natywnego Claude Code CLI supportu – trzeba dodać adapter. |
...
| **Cline / Roo Code** | **Build** (nie używać jako platformy) | Za lekkie, IDE-first, brak deterministycznego governance. Wzorzec spawn-owania Claude Code CLI można wykorzystać we własnym rozwiązaniu. |
...
| **SuperClaude** | **Skip** | Ciekawy pomysł (meta-programming configuration framework), ale za dużo bugów instalacyjnych, problemy z kompatybilnością macOS, community zbyt małe. |
| **Agent OS** | **Borrow** (wzorce spec-driven development) | Pomysł konwersji standardów na Claude Code Skills jest wart skopiowania. Sam framework za prosty dla potrzeb Octadecimal. |
...
**Opis:** OpenHands zapewnia Docker-runtime'y dla każdego zespołu agentów. Mastra dostarcza TypeScript-ową orkiestrację (workflows, agents, observability). Własny komponent Octadecimal Governance Layer implementuje rulebook z 3-poziomową eskalacją, wykorzystując Claude Code hooks i MCP.
...
### Rekomendacja 3: Build – własny, minimalistyczny orchestrator na Claude Code Agent Teams + MCP

**Opis:** Claude Code ma teraz natywne Agent Teams (od lutego 2026). Zamiast nakładać ciężki framework, zbudować lekki orchestrator w TypeScript, który: (a) definiuje zespoły jako CLAUDE.md z różnymi rulebookami, (b) używa MCP do komunikacji między zespołami, (c) implementuje 4-warstwową propagację wiedzy przez współdzielony vector store, (d) używa `colmena` (lub własnego rozwiązania) jako deterministycznej warstwy governance.

**Trade-off:** Najwięcej pracy, największe ryzyko, że zbudujesz coś, co już istnieje. Ale pełna kontrola nad architekturą i zero zależności od zewnętrznych roadmap. Claude Code Agent Teams mogą w 2027 uczynić zewnętrzne orchestratory zbędnymi – buildując własny, jesteś najbliżej metalu.
...
1. **Czy Claude Code PreToolUse hooks da się użyć do deterministycznego governance?** RFC #45427 udokumentował 4 failure modes: subagent bypass, silent hook failure, model self-modification, alternative tool paths. Bez odpowiedzi Anthropic na to RFC, każda warstwa governance zbudowana na hookach będzie dziurawa. **Hands-on test:** zbudować hook, który blokuje Write i próbować 100 sposobów obejścia.

2. **Jak bardzo stabilne jest OpenHands SDK?** Wydane w styczniu 2026, wciąż w fazie aktywnego developmentu. API delegacji dopiero jest przepisywane, by dorównać Claude Code Task tool. **Hands-on test:** zbudować agenta w OpenHands SDK, który deleguje do Claude Code przez ACP i zmierzyć stabilność na 100-task benchmarku.
...
| **Claude-Squad (bijutharakan)** | Ostatni commit ~6 miesięcy temu, brak licencji w README, tylko 1 autor | **HIGH** – prawdopodobnie abandoned |
| **BMAD-METHOD** | Ciężkie problemy z discoverability slash commands w Claude Code (5 osobnych issues), hardcodowana ścieżka `~/Dev/`, model "expansion packs" pachnie vendor lock-in | **MEDIUM** |
| **SuperClaude** | Bug kasujący lokalną instalację Claude Code, problemy z instalacją na macOS, community zbyt małe (głównie 1 maintainer) | **MEDIUM** |
...
| **CrewAI** | Python-only, brak Claude Code CLI integracji, governance zależne od zewnętrznych narzędzi (Galileo) | **MEDIUM** |
...
- Claude Flow → Ruflo historia: https://dev.to/stevengonsalvez/claude-flow-the-multi-agent-swarm-orchestrator-before-it-got-a-new-name-4kd4 (dostęp 2026-05-14)
...
- Claude Code Agent Teams: https://www.sitepoint.com/anthropic-claude-code-agent-teams/ (dostęp 2026-05-14)
- Claude Code governance RFC: https://github.com/anthropics/claude-code/issues/45427 (dostęp 2026-05-14)
- Claude Code "Fake Done": https://dev.to/catadef/fake-done-your-ai-coding-agent-says-it-finished-it-didnt-5b6f (dostęp 2026-05-14)
