...
- **Orkiestracja**: **Ruflo v3.5** (dawniej LLM-Flow) jako silnik multi-agent i MCP server.[file:95]
...
- **Interfejs**: automation assistant (Agent Teams, hooks, MCP).[file:95]
...
## 3. automation assistant + Agent Teams + hooks

### 3.1 Instalacja automation assistant

1. Zainstaluj VS Code.
2. W marketplace znajdź i zainstaluj rozszerzenie **automation assistant**.
3. Skonfiguruj klucz LLM provider (wg dokumentacji rozszerzenia / LLM platform).[file:95]

### 3.2 Włączenie Agent Teams i hooks

1. W ustawieniach projektu (np. `.llmsettings.json`) włącz:
...
3. W `AGENTS.md` opisz wysokopoziomowy setup (Ruflo, BMAD, AGT, Apple Container) jako kontekst dla automation assistant.[file:95]
...
### 4.3 Integracja Ruflo z automation assistant (MCP)

1. W konfiguracji automation assistant dodaj serwer MCP Ruflo:
   - host: `localhost`,
   - port: `PORT_RUFLO` (np. 8710),
   - schema z pliku konfiguracyjnego Ruflo (np. `mcp-config.json`).[file:95]

2. Skopiuj z repo Ruflo pliki integracyjne `AGENTS.md`, ewentualne przykłady skills, do swojego repo Octadecimal (.llmskills i/lub opis w głównym `AGENTS.md`).[file:95]
...
Następnie w automation assistant skonfiguruj MCP na `localhost:8200`.[file:95]
...
### 5.2 Integracja BMAD z automation assistant

1. Sklonuj integrację BMAD‑AT‑AGENTS:[file:95]

   ```bash
   git clone https://github.com/24601/BMAD-AT-AGENTS.git
   ```

2. Skopiuj skille/konfiguracje BMAD do swojego repo Octadecimal:
   - pliki `.chatmode.md` / `.skill.json` / inne z BMAD‑AT‑AGENTS do `.llmskills`.

3. W `.llmsettings.json` / `AGENTS.md` ustaw, że:
...
- `.llmskills/frontend.chatmode.md`, `backend.chatmode.md`, `security.chatmode.md` – definicje ról BMAD.[file:95]
...
### 6.2 Integracja AGT z automation assistant (hooks)
...
2. Dodaj mały serwis (Node/Python), który:
   ...
   - wystawia MCP/REST jako narzędzie „PersonalMemory” dla automation assistant.[file:95]

### 7.2 Warstwa 2 – pamięć projektu

- `project-context.md` – wypełnij opisem tech‑stacku, standardów, głównych reguł.
- `.llmskills` – trzymaj tam skille BMAD, Ruflo, governance.[file:95]
...
1. **Uruchom infrastrukturę**:
   ...
   Otwórz VS Code z aktywnym automation assistant (Agent Teams + hooks).
...
3. **W automation assistant**:
   - otwórz repo Octadecimal z BMAD (`project-context.md`, `.llmskills`, `docs/stories/`),
...
Jeżeli powyższy przepływ działa, masz skonfigurowane rekomendowane środowisko Octadecimal: macOS Tahoe + Apple Container, Ruflo jako silnik multi-agent, BMAD jako metodyka SDD, AGT jako policy kernel oraz automation assistant jako główne IDE.
