...
| **Brex** | Włączenie nietechnicznych członków zespołu (content designer) do procesu deweloperskiego i demokratyzacja dostępu do danych | Code review as primary mode: inżynierowie nie piszą, tylko recenzują zmiany wprowadzone przez Claude Code | Claude Code, MCP servers, Brex Explorer (text‑to‑SQL) | Architektura i kierunek definiowane przez człowieka; implementacja przez agenta | 3–4x poprawa w konkretnych zadaniach; content designer zrealizował projekt z backlogu (tygodnie/miesiące → kilka dni) |  |
...
| **TELUS** | Przyspieszenie developmentu bez wzrostu headcountu | 13 tys. customowych rozwiązań AI, integracja agentów z istniejącym workflow | Claude (custom solutions) | — | 30% szybszy development, 500k+ godzin zaoszczędzonych |  |
...
**Poziom 3: Agent‑initiated stops (agent sam prosi o pomoc).** Anthropic udokumentowało, że w najbardziej złożonych zadaniach Claude Code *sam* zatrzymuje się, aby zapytać o wyjaśnienia, ponad dwa razy częściej niż człowiek go przerywa. To nie jest oznaka słabości — to zaprojektowana cecha systemu, która zapobiega kosztownym błędom.

### Mechanizmy operacyjne nadzoru

- **Outcomes / grading.** Claude Managed Agents wprowadza oddzielnego „gradera”, który w osobnym kontekście ocenia każdy output względem zdefiniowanego rubryki i odsyła agenta do poprawki, dopóki nie spełni standardu. W benchmarkach Anthropic podniosło to skuteczność tasków nawet o 10 punktów procentowych na najtrudniejszych problemach.
...
> *„Effective oversight of agents requires more than putting a human in the approval chain.”* — Anthropic
...
2. **Delegacja i równoległa praca agentów (agent pracuje w tle).** Rakuten: „You can have five tasks running in parallel by delegating four to Claude Code while focusing on the remaining one”. OpenAI: wewnętrzna pętla (inner loop) ma górny limit jednej minuty — jeśli agent nie osiągnie postępu w tym czasie, coś jest nie tak z taskiem lub kontekstem.
...
- **Zadania agentowe** są oznaczane w backlogu i mogą być przypisywane agentom (GitHub pozwala przypisać issue do @claude lub @openhands, a agent otwiera draft PR).
...
- **Outcomes framework** (Claude Managed Agents): agent może przejść wiele iteracji, zanim output spełni rubric. Na wewnętrznych benchmarkach Anthropic lift skuteczności o 10 punktów na najtrudniejszych taskach.
...
- **Agent‑initiated stop** → agent prosi o wyjaśnienie (Claude Code robi to 2x częściej, niż człowiek go przerywa).
...
- **Orchestrator (lead agent)** — otrzymuje zadanie od człowieka, rozkłada na podzadania, deleguje do subagentów, zbiera wyniki. Claude Managed Agents dostarcza reference implementation.
...
- **Reviewer / grader agent** — działa w oddzielnym kontekście, ocenia output względem rubryki, wymusza poprawki. Outcomes framework Anthropic to gotowy wzorzec.
...
3. **Agent‑initiated stops** — agent ma pełne prawo i obowiązek zatrzymać się i zapytać, gdy czegoś nie rozumie (Claude Code: 2x częściej niż człowiek przerywa).
...
3. **Nie zakładaj, że agent zastąpi inżyniera** — badania Anthropic pokazują, że inżynierowie mogą w pełni delegować tylko 0–20% zadań; reszta wymaga nadzoru i walidacji.
...
- **Claude Managed Agents (Outcomes, Multiagent orchestration)** — gotowe wzorce do zaadaptowania
