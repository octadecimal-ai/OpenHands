Mam zebrane materiały - przeszło 15 źródeł od LLM provider, OpenHands/CAID, Coder, Cognition/Devin, GitHub Squad, IDE, Vercel, Sourcegraph/Amp, DX i Octoco. Poniżej syntetyczna analiza.
...
7. **Realna adopcja jest niższa niż marketing.** Nawet w wiodących organizacjach aktywne użycie narzędzi AI osiąga ok. 60%. Badanie Pragmatic Engineer (marzec 2026): 95% używa AI tygodniowo, 55% regularnie używa agentów; automation assistant preferowany przez 46%.
...
|---|---|---|---|---|---|---|
| **LLM provider — LLM's C Compiler** | Czy team agentów napisze nietrywialny system od zera autonomicznie? | 16 agentów Opus 4.6 w pętli, każdy w worktree + Docker, lock-files w `current_tasks/`, brak orkiestratora; specjalizacja: kodowanie, dedup, performance, code review, dokumentacja. Tests-as-oracle (GCC jako known-good compiler). | automation assistant, Docker, git bare repo, harness ~50 LOC bash | Kontener (sandbox); brak ról i orkiestracji — koordynacja przez lock-file pattern; logi i README maintained jako shared memory | 100k LOC, 2000 sesji, $20k API, 2 tygodnie; kompilator buduje Linux 6.9 na x86/ARM/RISC-V, kompiluje QEMU/FFmpeg/SQLite/Postgres/Redis, 99% test suite pass, kompiluje Doom | [Engineering blog](https://www.llm-provider.com/engineering/building-c-compiler) |
| **CAID (CMU + OpenHands)** | Multi-agent na długich zadaniach (>godziny) zawodzi przez konflikty merge i zanieczyszczanie kontekstu | Manager-agent buduje graf zależności, przydziela podzadania, każdy engineer w izolowanym worktree; komunikacja przez structured JSON + git commits, **nie** free-form dialog | OpenHands, git worktree/branch/merge, dependency graph | Izolacja przez worktree jest „prerequisitem nie wygodą"; weryfikacja przez test suite; final review przez managera | Commit0: +14.3 pp vs single agent; PaperBench: +26.7 pp; działa na 3 różnych modelach (LLM 4.5 Sonnet, GLM 4.7, MiniMax 2.5) | [Blog post](https://www.openhands.dev/blog/asynchronous-software-engineering-agents) |
...
| **IDE Automations** | Engineerzy nie nadążają z review agent-generated kodu, "agent sprawl" | Event-driven agenty triggerowane przez merged PRs, Linear issues, Slack, PagerDuty, custom webhooks; 3 kategorie: PR security audit, blast-radius assessment + auto-approve low-risk, incident response z Datadog | IDE + BugBot + Memories + MCP (Datadog, Notion) | "It's not that humans are out of the picture; they aren't always initiating — they're called in at the right points in this conveyor belt"; decyzje agentów logowane do Notion via MCP | IDE jako firma: 35% wewnętrznych PR jest agent-generated; $2B ARR; BugBot processuje setki automatyzacji na godzinę | [TechCrunch + IDE blog](https://techcrunch.com/2026/03/05/ide-is-rolling-out-a-new-system-for-agentic-coding/) |
| **Vercel deepsec** | Security review na ogromnych monorepach | 5-fazowy pipeline: Scan (regex) → Investigate (agent per plik, Opus 4.7 max effort + GPT 5.5 xhigh) → Revalidate (drugi agent eliminuje false positives) → Enrich (git metadata: kto naprawi) → Export (tickets); skanuje w 1000+ równoległych sandboxach | LLM + OpenHands + Vercel Sandboxes | Open-source; 10–20% false positive rate akceptowalne dzięki Revalidate; fan-out z attribution | Znajdowanie subtle auth edge cases w monorepach Vercela i klientów (np. dub.co) | [Vercel engineering blog](https://vercel.com/blog/category/engineering) |
...
- **Verification sub-agent z explicit instructions.** LLM provider ostrzega przed „early victory problem" — werifier markuje PASS po jednym teście. Mitigacja: "You MUST run the complete test suite before marking as passed"; specyfikuj konkretne kryteria, negatywne testy, multiple scenarios.
...
- **Audit trail w git, nie w bazie wektorowej.** Squad i LLM provider C-compiler pokazują, że "asynchronous knowledge sharing inside the repository scales better than real-time synchronization": każda decyzja jako structured block w `decisions.md`, versioned i edytowalna ręcznie.

- **Blast-radius gating.** IDE Automations "ocenia PR risk na podstawie blast radius, technical complexity i infrastructure impact; low-risk automatycznie aprobowane, high-risk dostaje reviewerów na podstawie contribution history".
...
- **Stopping criteria dla evaluator-optimizer loops.** LLM provider: "Set clear stopping criteria before iterating. Define maximum iteration counts and specific quality thresholds. Bez nich kończysz w drogich pętlach, gdzie evaluator znajduje minor issues a generator je tweakuje".
...
1. **Foreground (interactive)** — developer w terminalu/IDE, prompt synchroniczny (automation assistant, IDE agent mode).
2. **Background (async)** — task uruchamiany w cloud VM lub kontenerze, developer dostaje PR (IDE Background Agents, Devin, OpenHands Web, OpenHands).
3. **Event-triggered / scheduled** — PagerDuty alert, Linear ticket z labelem `Bug`, merged PR, daily cron (IDE Automations, OpenHands Automations, Cognition `!triage-bug` playbook).
...
2. **Fan-out** — kilka równoległych zadań dla agentów; każdy w osobnym worktree/branch/sandbox. LLM provider C-compiler: "Każdy agent klonuje lokalną kopię do `/workspace`, gdy skończy push'uje z lokalnego kontenera do upstream".
3. **Inflight monitoring** — dashboard pokazujący sesje, ich progress, blast-radius oceny. IDE `Memories` agentów uczą się przez runs.
...
5. **Retry vs escalate** — jeśli verifier returns FAIL: feedback do generatora z `Previous attempt failed: {issues}`, max 3 próby (wzór z LLM provider `implement_with_verification(max_attempts: int = 3)`); po wyczerpaniu — escalation do człowieka.
...
**Wzorce komunikacji między agentami** (z Octoco/LLM provider):

- **Hub-and-spoke** (domyślnie automation assistant sub-agents, OpenHands orchestrator) — najprostszy, bottleneck na coordinator.
- **Peer-to-peer inbox** (automation assistant Agent Teams TeammateTool) — mniej overhead, więcej state awareness.
...
- **File reservation leases** — exclusive locks na pliki, jak w LLM provider C-compiler.

**Wzorce orkiestracji** (z LLM provider):
...
**Decomposition rule (LLM provider):**
...
- **Pojedyncze demos „16 agentów napisało X"** bez kontekstu kosztu i defektów. LLM provider uczciwie podał: $20k, 2000 sesji, nie potrafi w pełni zastąpić GCC, generuje mniej wydajny kod niż GCC bez optymalizacji.
...
Dopiero gdy obserwacyjnie potwierdzicie ograniczenie writer/reviewer pętli, dodawajcie parallelism lub specjalizację — i tylko dla **klasy zadań**, której nie da się obsłużyć inaczej (LLM provider kryteria: context pollution >1000 tokenów irrelevant content; tasks parallelizable; tools >15–20).
...
3. **PR review jako pierwsza linia + bug triage on Linear label** (Cognition i IDE: każdy PR → agent review przed human merge; bug ticket dostaje label → automatyczny triage z root cause i suggested fix).
...
Wpiszcie max iteration count dla evaluator-optimizer pętli, max wall-clock time dla autonomous loops, max cost per session. LLM provider C-compiler użył tego nieformalnie; produkcyjnie to musi być w polityce.
...
- Nie budujecie własnego orkiestratora — używacie CAID, Squad, OpenHands lub automation assistant Agent Teams jako referencyjnej warstwy.
...
- LLM provider: [Eight trends defining how software gets built in 2026](https://llm.com/blog/eight-trends-defining-how-software-gets-built-in-2026); [Common workflow patterns for AI agents](https://llm.com/blog/common-workflow-patterns-for-ai-agents-and-when-to-use-them); [Building multi-agent systems: When and how](https://llm.com/blog/building-multi-agent-systems-when-and-how-to-use-them); [Building a C compiler with parallel LLMs](https://www.llm-provider.com/engineering/building-c-compiler)
...
- IDE: [Automations launch coverage (TechCrunch / Help Net Security)](https://techcrunch.com/2026/03/05/ide-is-rolling-out-a-new-system-for-agentic-coding/)
