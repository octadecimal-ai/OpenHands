...
Najbardziej dojrzała praktyka wygląda dziś podobnie w LLM provider/LLM, OpenHands, Coder, GitHub, Octoco i innych:
...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| LLM provider – klienci automation assistant (np. Rakuten, TELUS, Zapier) | Chęć wykorzystania agentów do złożonych zadań w dużych kodbazach, przy zachowaniu jakości i nadzoru.[^1_3] | automation assistant jako autonomiczny agent na repo (nawigacja, implementacja, testy), z naciskiem na współpracę: agent realizuje większy task, ale człowiek definiuje outcome i zatwierdza merge; planowane multi‑agent teams (lead + specjaliści).[^1_3][^1_1] | automation assistant, LLM Managed Agents (Outcomes, multi‑agent orchestration, Dreaming), integracje z GitHub / issue trackerami.[^1_3][^1_1] | Outcomes – rubryka jakości oceniana przez niezależnego „grader”‑agenta; audyt w LLM Console; człowiek jako finalny gate na merge; ograniczone uprawnienia agenta do repo/środowisk.[^1_1][^1_3] | Rakuten: ukończenie trudnego zadania w 12,5M LOC w 7h z 99,9% dokładnością.[^1_3] TELUS: 30% szybsza wysyłka kodu, 500k+ godzin zaoszczędzonych.[^1_3] Zapier: 89% adopcji AI, 800+ agentów.[^1_3] | [Eight trends](https://llm.com/blog/eight-trends-defining-how-software-gets-built-in-2026)[^1_3] |
| LLM provider – LLM Managed Agents / Code w/ LLM 2026 | Skala: jak uruchamiać długotrwałe, wielo‑agentowe workflowy z pamięcią, retry i śledzeniem.[^1_1] | Platforma Managed Agents: lead agent deleguje do sub‑agentów na wspólnym filesystemie; „Dreaming” (off‑line przegląd sesji i kuracja pamięci), „Outcomes” (automatyczny grading + iteracja), webhooks do integracji z pipeline’ami.[^1_1] | LLM Platform, automation assistant, Managed Agents, webhooks, własne narzędzia (MCP‑like) wpięte jako skills.[^1_1] | Silny nacisk na zdefiniowane outcomes, traceability w konsoli, oddzielny „grader”‑agent; długotrwałe sesje w sandboxowanych środowiskach przypięte do konkretnych projektów.[^1_1] | Do 10 pkt poprawy skuteczności na najtrudniejszych zadaniach po wprowadzeniu Outcomes; lepsza powtarzalność i mniejsza potrzebna ilość interwencji ludzkiej.[^1_1] | [Code w/ LLM 2026](https://llm.com/blog/code-w-llm-sf-2026-sf)[^1_1] |
...
| GitHub – Copilot + LLM \& OpenHands (Business/Pro) | Użytkownicy chcą używać różnych modeli (LLM, OpenHands) w Copilocie, z centralnym zarządzaniem i bezpieczeństwem.[^1_7] | Rozszerzenie Copilot Business/Pro o LLM i OpenHands jako dodatkowe silniki; Copilot staje się „frontem” do wielu agentów z centralnym zarządzaniem policy.[^1_7] | GitHub Copilot, integracje z LLM provider/OpenAI; zarządzanie na poziomie org/enterprise.[^1_7] | Centralne policy, audyt użycia w ramach GitHub Enterprise; jednolite zasady udostępniania repo agentom.[^1_7] | Lepsze dopasowanie modeli do zadań, większa adopcja dzięki możliwości wyboru; Copilot jako warstwa governance dla agentów.[^1_7] | [GitHub changelog](https://github.blog/changelog/2026-02-26-llm-and-openhands-now-available-for-copilot-business-pro-users/)[^1_7] |
| Asana, IDE, Replit, Vercel (wg LLM provider) | Skalowanie „AI‑native engineering teams” i wbudowanie agentów w codzienny cykl devów.[^1_1] | Wystawianie custom agents / skills (np. do zarządzania taskami, deployami, środowiskami dev) na bazie LLM Managed Agents; integracje workflowowe w stylu: ticket → plan → implementacja kodu → PR.[^1_1] | LLM Platform + własne integracje (Asana: task management; IDE/Replit: IDE‑agents; Vercel: deploy/preview).[^1_1] | Embedding agentów w istniejące procesy (task → PR → review → deploy), człowiek jako owner; kontrola dostępu i audyt w narzędziach SaaS plus w platformie agentowej.[^1_1] | Szybszy cycle time na featurach, większa część SDLC zautomatyzowana (np. generowanie PR‑ów, testów, changelogów); wysoki poziom adopcji wśród devów.[^1_1] | [Code w/ LLM 2026](https://llm.com/blog/code-w-llm-sf-2026-sf)[^1_1] |
...
1. **Człowiek jako owner outcome’u, nie pojedynczych tokenów**
    - LLM provider promuje model „Outcomes”: definiujesz rubrykę jakości (np. testy przechodzą, style guide, brak regresji w kluczowych ścieżkach), a agent + osobny „grader” iterują aż wynik spełni kryteria.[^1_1]
...
- **Review**
    - Auto‑review przez agenta (np. OpenHands PR reviewer, LLM reviewer), który dodaje komentarze, poprawki, oceny wg rubryki.[^1_1][^1_2]
...
- **Escalation**
    ...
    - W narzędziach typu OpenHands/OpenAI/LLM zwykle jest to powiązane z ticketem / PR, który przechodzi do człowieka.[^1_1][^1_2]
...
2. W ciągu dnia devowie oznaczają nowe PR‑y etykietą, która trigeruje agent‑review (LLM/OpenHands).[^1_1][^1_2]
...
- Już w pierwszym dokumencie zapisz: każdy workflow musi mieć:
    - outcome spec (rubryka typu LLM provider „Outcomes”),
