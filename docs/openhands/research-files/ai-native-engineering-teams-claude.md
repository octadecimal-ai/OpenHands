...
|---|---|---|---|---|---|---|
| **Anthropic — C compiler (Feb 2026)** | Demo długo-horyzontowej autonomii: czy 16 agentów napisze działający kompilator C w Ruście od zera? | Single-loop harness ("Ralph loop") w pętli `while true`; każdy agent w osobnym kontenerze Docker z własnym klonem repo; lock-file w `current_tasks/` jako sync; GCC jako "known-good oracle" dla parallelizacji długich tasków; specjalizowane role (refactor, performance, docs, review) | Claude Opus 4.6 + Claude Code, Docker, git, własny harness, CI z testami | Pełna autonomia w sandboksie; brak orchestratora; każda zmiana musi przejść stricter CI; aktualizowane README/PROGRESS.md jako pamięć między sesjami | 100 000 linii Rusta, kompiluje Linux 6.9, ~2000 sesji, ~20 000 USD; 99% pass na GCC torture test suite | [anthropic.com](https://www.anthropic.com/engineering/building-c-compiler) |
...
| **Cursor — Automations (Mar 2026)** | "Prompt-and-monitor" nie skaluje się; review i maintenance nie nadążają za produkcją kodu | Event-driven agenty: trigger ze Slacka, Linear, GitHub PR merged, PagerDuty incident, webhook lub schedule; każda automation odpala cloud sandbox z własnym zestawem MCP i modelu | Cursor Cloud Agents, Bugbot, MCP (Datadog, Notion), GitHub Actions | Każdy agent ma zdefiniowany cel + scope; security review na każdy push do main, ale nie blokuje PR; risk-based auto-approve dla low-blast-radius zmian; log do Notion dla audytu | Bugbot resolution rate: 52% → 80% (LLM-judge na public repos); ~2 mln PR-ów review/miesiąc; 35% Bugbot Autofix mergowanych; ~1 USD/PR | [cursor.com](https://cursor.com/blog/automations), [cursor.com](https://cursor.com/blog/building-bugbot) |
...
| **CAID (akademicki, CMU + OpenHands, Apr 2026)** | Multi-agent breaks na integracji: dwóch agentów pisze poprawny kod, który po mergu się sypie | Manager agent buduje dependency graph; każdy engineer agent w izolowanym git worktree; integracja przez `git merge`; cała komunikacja przez structured JSON + commity, nie free-form chat | OpenHands + Claude 4.5 Sonnet / GLM 4.7 / MiniMax 2.5 | Test suite jako automated verifier na każdym kroku; ablation pokazała, że bez izolacji worktree wynik istotnie spada | +14.3% absolute na Commit0; +26.7% na PaperBench vs single-agent baseline | [openhands.dev](https://www.openhands.dev/blog/asynchronous-software-engineering-agents) |
| **Coder Agents (May 2026)** | Każdy zespół używa innego agenta z innym providerem — fragmentacja, lock-in, brak kontroli sieci | Provider-agnostic agent runtime w infrastrukturze klienta (self-hosted/air-gapped); centralne zarządzanie modelami, promptami, MCP, skills; network-isolated workspaces | Coder Workspaces + Coder Agents + AI Governance (LLM gateway) | Centralizacja modeli i promptów; on-prem; opcjonalnie third-party agenci (Claude Code, OpenHands) działają wewnątrz Coder Workspaces z gateway'em | Brak publicznych liczb; case: U.S. defense intelligence org, 2500+ developerów | [coder.com](https://coder.com/blog/introducing-coder-agents) |
| **GitHub Copilot — Agent Control Plane (Feb 2026)** | Multi-vendor: Claude, OpenHands, Copilot — różne UI, różny audit, różna polityka | Unified Agent Control Plane: Claude, OpenHands i Copilot współdzielą memory, repo context, instructions/policies; enable per-org/per-repo; sessions widoczne jako draft PR | GitHub Copilot, Claude, OpenAI OpenHands, VS Code 1.109+ | Enterprise AI Controls → Agents, centralne policy management, audit logging; każda sesja = jeden premium request | Cele operacyjne — porównanie podejść (assign issue do wielu agentów), nie metryki productivity | [github.blog](https://github.blog/changelog/2026-02-26-claude-and-openhands-now-available-for-copilot-business-pro-users/) |
| **Rakuten (Anthropic case study)** | Refaktor w 12.5 mln linijek kodu (vLLM): activation vector extraction | Single-agent Claude Code w pełni autonomicznie przez kilka godzin | Claude Code, Opus | Pełny audit ścieżki kodu; numeryczna walidacja na końcu | Task zakończony w 7h pracy autonomicznej, 99.9% accuracy numerycznej | [claude.com/customers/rakuten](https://claude.com/customers/rakuten) |
| **TELUS (Anthropic case study)** | Demokratyzacja AI w całej organizacji | 13 000+ custom AI solutions zbudowanych wewnętrznie | Claude + Claude Code | Wewnętrzny program governance dla "every employee builds" | 30% szybszy ship engineering code, 500 000 godzin saved | [claude.com/customers/telus](https://claude.com/customers/telus) |
| **Doctolib (healthcare, Anthropic case study)** | Legacy testing infrastructure | Wymiana wewnętrzną z użyciem Claude Code w skali zespołu | Claude Code | Standardowy code review + ich własny SDLC | Testing infra wymieniona w godziny zamiast tygodni; 40% szybszy shipping features | [claude.com/blog/how-enterprises-are-building-ai-agents-in-2026](https://claude.com/blog/how-enterprises-are-building-ai-agents-in-2026) |
| **Litera (Anthropic / Cognition)** | Każdy engineering manager dostaje "team of Devins" w roli QE/SRE/DevOps | Każdy EM ma fleet Devinów do QE testing | Devin | Standardowy human review | +40% test coverage; 93% szybsze regression cycles | [cognition.ai](https://cognition.ai/blog/devin-annual-performance-review-2025) |
...
**Verifier-as-oversight.** Najpoważniejsze zespoły inwestują w testy/CI/lintery/security scannery zanim w agentów. Carlini z Anthropic powiedział to wprost przy projekcie kompilatora: harness pisze się dla Claude, nie dla człowieka — logi muszą być greppable, błędy oznaczone `ERROR` w jednej linii, agregaty pre-policzone, kontekst nie zanieczyszczony tysiącami bajtów. CAID idzie dalej i traktuje test suite jako jedyne źródło prawdy między agentami: cała komunikacja przez structured JSON + git commity, nigdy free-form chat. To jest sedno: jeśli verifier jest słaby, agenci rozwiążą zły problem.

**Writer/reviewer loop jako structural primitive.** Devin Review robi to dla każdego PR-a Cognition; Cursor Bugbot dla 2 mln PR-ów miesięcznie. Cognition podaje konkretny efekt: po wprowadzeniu strukturalnej pętli review-fix-recheck merge rate wzrósł z 34% do 67%. Bugbot publikuje resolution rate (procent flagów które developer faktycznie naprawia przed mergem) — to anty-vanity metryka, bo karze fałszywie pozytywne i pozwala odróżnić "louder" od "more accurate".

**Risk-based auto-approve.** Cursor opisuje wewnętrzną automation, która ocenia każdy PR po blast radius, complexity i infrastructure impact — low-risk zmiany auto-approve, high-risk eskalują do człowieka, decyzje logowane do Notion. To skaluje review, którego ludzie inaczej nie nadążą wykonać.
...
| **Resolution rate komentarzy reviewera-agenta** | % flagów Bugbot/Devin Review, które developer faktycznie naprawia przed mergem | Cursor: 52% → 80% (LLM-judge na public repos) — anty-vanity, bo karze fałszywie pozytywne |
...
| **Cost per successful task** | USD per task, który człowiek zaakceptował | Cursor: ~1 USD per Bugbot run; Anthropic compiler: 20 000 USD / 100 000 linii (jednorazowo, dlatego ekonomicznie niedobrze) |
...
- **Liczba zaadoptowanych "agent teammates" / instancji** — Anthropic chwali się 13 000 custom AI solutions w TELUS, ale to liczba narzędzi, a nie outcome. Zespół powinien dodatkowo mierzyć, ile z tych 13 000 jest faktycznie używanych po 30 dniach.
...
- [Anthropic — Building a C compiler with parallel Claudes (Feb 2026)](https://www.anthropic.com/engineering/building-c-compiler)
- [Anthropic / Claude — Common workflow patterns for AI agents (Mar 2026)](https://claude.com/blog/common-workflow-patterns-for-ai-agents-and-when-to-use-them)
...
- [Cursor — Automations (Mar 2026)](https://cursor.com/blog/automations)
- [Cursor — Building Bugbot](https://cursor.com/blog/building-bugbot) i [Bugbot self-improves with learned rules](https://cursor.com/blog/bugbot-learning)
- [GitHub Blog — Claude and OpenHands in Copilot (Feb 2026)](https://github.blog/changelog/2026-02-26-claude-and-openhands-now-available-for-copilot-business-pro-users/)
...
- [Anthropic / Claude — Eight trends defining how software gets built in 2026](https://claude.com/blog/eight-trends-defining-how-software-gets-built-in-2026)
