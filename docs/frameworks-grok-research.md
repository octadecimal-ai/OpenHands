...
**Critical-path MCPs dla MVP** (pierwsza działająca wersja multi-agent na Claude Code + Docker):
...
- **Architektura**: TypeScript MCP servers, per-agent credential scoping, Docker isolation, monitoring via Langfuse/Grafana (istniejące). Testować w Claude Code, skupić się na auth security i rate-limit resilience.
