...
Poniższa tabela jest syntetyczną oceną pod kątem **Octadecimal**: one-person software house, macOS host, TypeScript MCP servers, Python research agents, automation assistant i Docker-friendly multi-container local orchestration.
...
| Opcja | Performance | Compatibility with automation assistant | Developer ergonomics | Observability | Security | Resource efficiency |
|---|---:|---:|---:|---:|---:|---:|
...
Moja praktyczna estymacja dla **24 GB RAM** jest taka. Dla **lightweight LLM Agent SDK / własnych TS-Python agents** traktuj **1–1,5 GB RAM** jako realistyczny budżet na jednego aktywnie pracującego execute-heavy workera, bo LLM providerowe 1 GiB to sensowny baseline, a nie twardy ceiling. Dla **OpenHands-class heavier sandboxes** licz raczej **2–4 GB na aktywną jednostkę**, bo sam produkt rekomenduje minimum 4 GB dla lokalnego uruchomienia i ma Docker workspace overhead. Z tej perspektywy **7 logicznych teamów** możesz modelować bez problemu, ale **7 aktywnych sandboxów równocześnie** na 24 GB to zły pomysł; rozsądny laptopowy scheduler powinien trzymać **3–5 hot executors** i resztę w stanie queued / dormant. To jest wniosek inferencyjny z dokumentacji LLM provider i OpenHands, a nie oficjalna liczba vendorów. citeturn18view0turn22view3
...
```mermaid
flowchart LR
    A[macOS Tahoe host] --> B[automation assistant + MCP control plane on host]
...
2. Uruchom lokalnie:
   - **LLM Agent SDK** w TypeScript i Python,
   - **MCP servers** w TS na hoście i w kontenerze,
   - **OpenHands** z Docker sandbox,
   - prosty **LangGraph** workflow z persistence,
   - jeden stack w **Dev Container** z automation assistant. citeturn18view0turn16view2turn22view2turn25view2turn18view5
3. Włącz telemetry: **automation assistant OTEL**, **OpenHands OTEL**, ewentualnie **Cline OTEL** i trace’y LangSmith/LangGraph. To od razu pokaże, czy twoje escalation flows i inter-agent comms są obserwowalne wystarczająco dobrze. citeturn16view1turn22view1turn28view0turn25view0
