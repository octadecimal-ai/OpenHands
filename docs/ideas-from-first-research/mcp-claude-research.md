# Rekonesans ekosystemu MCP dla platformy multi-agent na Claude Code + Docker (maj 2026)
...
| **Indeed** | `mcp.indeed.com/claude/mcp` (oficjalny remote, obecnie tylko Claude Connector) | `borgius/jobspy-mcp-server` (python-jobspy, multi-board) | medium-high | (oficjalny: niejawny), MIT (jobspy) | **use** — oficjalny gdy działa, jobspy fallback |
...
- **Slack reference server** — używaj wyłącznie zencoderai/korotovsky lub oficjalnego remote; reference Anthropic ma niepatched data-exfiltration via link unfurling (Embrace The Red, maj 2025).
...
- Indeed MCP (oficjalny, gdy ich Claude Connector zostanie open beta)
...
- Jeśli Anthropic wprowadzi remote MCP standard z built-in per-agent identity (a nie tylko OAuth user-level) → przesuwa wszystkie OAuth-only servery w stronę "use without `mcp-remote`".
...
- Anthropic w grudniu 2025 przekazał MCP do Agentic AI Foundation (AAIF) — directed fund pod Linux Foundation, współzałożony z Block i OpenAI. Dalsze "official servery" mogą teraz pochodzić od AAIF, nie tylko Anthropic. Monitoruj registry pod `modelcontextprotocol.io`.
...
| Anthropic archived servers note | ✅ |
